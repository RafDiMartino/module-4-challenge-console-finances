...
for (let i = 0; i < finances.length; i++) {
    if (i > 0) {
        ...
        if ((finances[i][1] - finances[i - 1][1]) < minProfit) {
            minProfit = (finances[i][1] - finances[i - 1][1])
            minProfitMonth = (finances[i][0])
        }
        totalChangeInProfits += (finances[i][1] - finances[i - 1][1])
    }
    totalProfit += finances[i][1]
}

let averageChangeInProfits = totalChangeInProfits / (finances.length - 1)
